...
if __name__ == "__main__":
    ...
    ml_data = ExampleData.ml_training_curves(epochs=100, learning_rates=[0.001, 0.01, 0.1])
    ...
    with FigureManager(rows=2, cols=2, figsize=(16, 12)) as fm:
        ...
        fm.line(0, 0, ml_data, x="epoch", y=["train_loss", "val_loss"],
        ...
        fm.line(0, 1, ml_data, x="epoch", y="val_loss",
               hue_by="learning_rate", 
               title="Validation Loss by Learning Rate")

        # Accuracy progression
        fm.line(1, 0, ml_data, x="epoch", y=["train_accuracy", "val_accuracy"],
        ...
        for _, row in final_epoch.iterrows():
            performance_data.extend([
                {"learning_rate": row["learning_rate"], "metric": "train_loss", "value": row["train_loss"]},
                {"learning_rate": row["learning_rate"], "metric": "val_loss", "value": row["val_loss"]},
                {"learning_rate": row["learning_rate"], "metric": "train_accuracy", "value": row["train_accuracy"]},
                {"learning_rate": row["learning_rate"], "metric": "val_accuracy", "value": row["val_accuracy"]}
            ])
        
        import pandas as pd
        perf_df = pd.DataFrame(performance_data)
        
        fm.bar(1, 1, perf_df, x="learning_rate", y="value", hue_by="metric",
