...
if __name__ == "__main__":
    ...
    with FigureManager(rows=2, cols=2, figsize=(12, 10)) as fm:
        ...
        fm.hist(0, 0, basic_data, x="values", bins=20, 
               title="Basic Histogram")

        # Density histogram
        fm.hist(0, 1, basic_data, x="values", bins=20, density=True,
               title="Density Histogram")

        # Multiple distributions
        multi_data = ExampleData.distribution_data(distributions=3)
        fm.hist(1, 0, multi_data, x="value", hue_by="distribution", bins=20,
               title="Multiple Distributions", alpha=0.7)

        # Custom styling
        fm.hist(1, 1, basic_data, x="values", bins=30, 
               title="Custom Styling", color="red", alpha=0.6)
