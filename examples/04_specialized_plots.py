...
def main(args):
    with FigureManager(rows=2, cols=2, figsize=(16, 12)) as fm:
        fm.fig.suptitle("Example 4: Specialized Plots - Heatmap, Contour, and Distribution Types", fontsize=16)
        ...
        fm.plot("heatmap", 0, 0, heatmap_data,
            x="column", y="row", values="value", # REQUIRED: heatmap data mapping
        ...
        contour_data = ExampleData.gaussian_mixture(n_components=2, n_samples=400, seed=402)
        assert 'x' in contour_data.columns
        assert 'y' in contour_data.columns
        assert len(contour_data) == 400

        fm.plot("contour", 0, 1, contour_data,
            x="x", y="y",                        # REQUIRED: 2D data mapping
        ...
        violin_data = ExampleData.grouped_categories(n_categories=4, n_groups=2, n_per_combo=25, seed=403)
        ...
        fm.plot("violin", 1, 0, violin_data,
            x="category", y="value",             # REQUIRED: categorical data mapping
        ...
        hist_data = ExampleData.distribution_data(n_samples=500, distributions=1, seed=404)
