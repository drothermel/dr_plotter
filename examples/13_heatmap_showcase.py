...
if __name__ == "__main__":
    ...
    with FigureManager(rows=1, cols=2, figsize=(15, 6)) as fm:
        ...
        verification_result = verify_legend_visibility(
            fm.fig,
            expected_visible_count=0,  # We expect 0 legends (heatmaps don't use discrete legends)
            fail_on_missing=False,  # Don't fail for missing legends since we expect 0
        )

        if verification_result["visible_legends"] > 0:
            print("\n💥 EXAMPLE 13 FAILED: Unexpected legends detected!")
            print(
                "   - Expected 0 legends (heatmaps use colorbars, not discrete legends)"
            )
            print(
            ...
            print("   Heatmaps should use colorbars, not discrete legends.")
        ...
        print(
            "\n🎉 SUCCESS: No unexpected legends found - heatmaps are clean as expected!"
        )
