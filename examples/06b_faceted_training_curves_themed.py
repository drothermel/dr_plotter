...
def create_model_size_ordering() -> List[str]:
    size_order = ["4M", "6M", "8M", "10M", "14M", "16M", "20M", "60M", "90M", "150M", "300M", "530M", "750M", "1B"]
    return size_order

def create_faceted_training_curves_theme(x_log: bool = False, y_log: bool = False) -> Theme:
    ...
    return Theme(
        ...
        **{
            consts.get_cycle_key("hue"): itertools.cycle(color_palette[:len(model_sizes)]),
        }
    )

def subset_data_for_plotting(df: pd.DataFrame, target_recipes: List[str], model_sizes: List[str]) -> pd.DataFrame:
    target_metrics = ["pile-valppl", "mmlu_average_correct_prob"]
    
    assert all(metric in df.columns for metric in target_metrics), f"Missing metrics from {target_metrics}"
    assert all(recipe in df["data"].values for recipe in target_recipes), f"Missing recipes from {target_recipes}"
    ...
    filtered_df["data"] = pd.Categorical(filtered_df["data"], categories=target_recipes, ordered=True)
    filtered_df = filtered_df.sort_values(["params", "data", "step"])
    
    return filtered_df

def create_faceted_grid_with_theme(custom_theme: Theme, num_recipes: int, num_model_sizes: int) -> Tuple[plt.Figure, Any]:
    ...
    fm.fig.suptitle(f"Themed Faceted Training Curves: 2 Metrics × {num_recipes} Data Recipes", fontsize=16, y=0.96)
    return fm.fig, fm

def plot_training_curves_themed(df: pd.DataFrame, target_recipes: List[str], x_log: bool = False, y_log: bool = False, xlim: Tuple[float, float] = None, ylim: Tuple[float, float] = None) -> None:
    num_model_sizes = len(df["params"].cat.categories)
    custom_theme = create_faceted_training_curves_theme(x_log=x_log, y_log=y_log)
    fig, fm = create_faceted_grid_with_theme(custom_theme, len(target_recipes), num_model_sizes)
    ...
    plt.tight_layout()
    plt.savefig("examples/plots/06b_faceted_training_curves_themed.png", dpi=150, bbox_inches='tight')
    plt.show()

def create_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Themed Faceted Training Curves - 2 Metrics × N Data Recipes")
    parser.add_argument("--x-log", action="store_true", help="Use log scale for X-axis (training steps)")
    parser.add_argument("--y-log", action="store_true", help="Use log scale for Y-axis (metric values)")
    parser.add_argument("--save", action="store_true", help="Save plot instead of displaying")
    
    # Filtering and ordering
    parser.add_argument("--recipes", nargs="+", default=["C4", "Dolma1.7", "FineWeb-Edu", "DCLM-Baseline"],
                       help="Data recipes to include (in order for subplot columns)")
    parser.add_argument("--model-sizes", nargs="+", 
                       default=["4M", "6M", "8M", "10M", "14M", "16M", "20M", "60M", "90M", "150M", "300M", "530M", "750M", "1B"],
...
def main() -> None:
    ...
    custom_theme = create_faceted_training_curves_theme(x_log=args.x_log, y_log=args.y_log)
    ...
    print(f"Theme axes scales: X={axes_info.get('xscale', 'linear')}, Y={axes_info.get('yscale', 'linear')}")
    
    print(f"Creating themed faceted training curves visualization with {config_desc}...")
    plot_training_curves_themed(filtered_df, args.recipes, x_log=args.x_log, y_log=args.y_log, xlim=args.xlim, ylim=args.ylim)
