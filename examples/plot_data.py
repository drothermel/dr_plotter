...
class ExampleData:
    ...
    def get_all_plot_types_data() -> Dict[str, pd.DataFrame]:
        ...
        bar_data = ExampleData.categorical_data(n_categories=5, n_per_category=1, seed=102)
        bar_data["category_group"] = ["Group_1", "Group_2", "Group_1", "Group_2", "Group_1"]
        
        histogram_data = ExampleData.distribution_data(n_samples=400, distributions=2, seed=103)
        
        violin_data = ExampleData.grouped_categories(n_categories=3, n_groups=2, n_per_combo=25, seed=104)
        
        heatmap_data = ExampleData.heatmap_data(rows=6, cols=5, seed=105)
        
        contour_data = ExampleData.gaussian_mixture(n_components=2, n_samples=300, seed=106)
    ...
    def get_color_coordination_data() -> Dict[str, pd.DataFrame]:
        ...
        scatter_data["category"] = np.tile(shared_categories, len(scatter_data) // 4)[:len(scatter_data)]
        
        line_data = ExampleData.time_series_grouped(periods=25, groups=4, seed=201)
        line_data["series"] = line_data["group"].map({
            "Group_A": "Alpha", "Group_B": "Beta", "Group_C": "Gamma"
        }).fillna("Delta")
        
        violin_data = ExampleData.grouped_categories(n_categories=3, n_groups=4, n_per_combo=15, seed=202)
        violin_data["group"] = violin_data["group"].map({
            "Group_1": "Alpha", "Group_2": "Beta", "Group_3": "Gamma", "Group_4": "Delta"
        }).fillna("Alpha")
        
        bar_data = ExampleData.categorical_data(n_categories=4, n_per_category=1, seed=203)
        bar_data["category"] = shared_categories
        
        histogram_data = ExampleData.distribution_data(n_samples=300, distributions=4, seed=204)
        dist_map = {"Normal": "Alpha", "Skewed": "Beta", "Bimodal": "Gamma"}
        histogram_data["distribution"] = histogram_data["distribution"].map(dist_map).fillna("Delta")
    ...
    def get_individual_styling_data() -> Dict[str, pd.DataFrame]:
        ...
        scatter_data["category"] = np.random.choice(["Type_A", "Type_B", "Type_C"], len(scatter_data))
        
        line_data = ExampleData.time_series_grouped(periods=40, groups=4, seed=301)
        
        violin_data = ExampleData.grouped_categories(n_categories=3, n_groups=3, n_per_combo=20, seed=302)
        
        bar_data = ExampleData.categorical_data(n_categories=5, n_per_category=1, seed=303)
        bar_data["category"] = ["A", "B", "C", "D", "E"]
        
        histogram_data = ExampleData.distribution_data(n_samples=400, distributions=1, seed=304)
