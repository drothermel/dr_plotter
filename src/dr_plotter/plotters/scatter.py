...
class ScatterPlotter(BasePlotter):
    ...
    def __init__(self, data, x, y, hue=None, size=None, marker=None, alpha=None, **kwargs):
        """
        ...
        if marker is not None and isinstance(marker, str) and marker not in self.plot_data.columns and marker not in [METRICS, METRICS_STR]:
    ...
    def _render_grouped(self, ax):
        ...
        group_styles = self._get_group_styles(
            self.plot_data, self.hue, None, self.size, marker=self.marker, alpha=self.alpha
        )
        ...
        if group_cols:
            ...
            for name, group_data in grouped:
                ...
                plot_kwargs = {
                    ...
                    's': self._get_style('marker_size', 50) * styles.get('size_mult', 1.0),
