...
def discover_categorical_dimensions(data: pd.DataFrame, config: FacetingConfig) -> set[str]:
    ...
    for col in data.columns:
        ...
        is_categorical = (
            data[col].dtype == 'object' or  # String columns
            data[col].dtype.name == 'category' or  # Explicit categorical dtype
            unique_count < 20 or  # Low cardinality (configurable threshold)
            unique_count / total_count < 0.05  # Less than 5% unique values
        )
...
def validate_dimensions(data: pd.DataFrame, config: FacetingConfig) -> tuple[set[str], set[str]]:
...
def interactive_dimension_validation(data: pd.DataFrame, config: FacetingConfig, 
